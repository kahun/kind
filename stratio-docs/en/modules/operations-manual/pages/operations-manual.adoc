...
To upgrade the _Stratio Cloud Provisioner_ version from 0.2 to 0.3 or from a 0.3 patch version to 0.the last 0.3 stable you must run the script _upgrade-provisioner++_++.py_.
...
The cluster.yaml and secrets.yml files must be present in the workspace. If the cluster.yaml file has a different name or is located in a different path, it can be specified using the -d parameter.
