...
Si nos referimos al control-plane, y como es evidente, en EKS y AKS no se podrá indicar una imagen, pero en AWS no-gestionado, Azure no-gestionado y en GCP si, siendo obligatorio en AWS no-gestionado para versiones de Kubernetes 1.27 o superiores en el caso de usar ECR como registry y en GCP para todas las versiones.

Para los nodos workers, es opcional en todos los casos (al no indicarla, el controller asigna una disponibilizada por el _cloud provider_) menos AWS no-gestionado para versiones de Kubernetes 1.27 o superiores en el caso de usar ECR como registry y GCP para todas las versiones, donde también será obligatorio especificarla ya que no disponibiliza ninguna imagen.
