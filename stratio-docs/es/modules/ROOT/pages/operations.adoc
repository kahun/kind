...
Las nuevas instancias se pueden ver en 'VM Scale set' -> 'Instances'. Este cambio no se reflejará en el parámetro _quantity_ del grupo de nodos del objeto _KeosCluster_.
...
Esta operación se realiza en el API Server, siendo los _controllers_ los encargados de crear las VM en el proveedor de _Cloud_ y agregarlas al _cluster_ como nodos _worker_ de Kubernetes.
...
Se deberán asegurar los permisos necesarios en el directorio backup de la máquina bastión para que el usuario que ejecuta el script pueda escribir en él (se crea el directorio ./backup/upgrade/).
...
La actualización del _cluster_ a una versión superior de Kubernetes se realizará en dos partes dentro del mismo proceso atómico: primero el _control-plane_ y, una vez que esté en la nueva versión, se procederá a la actualización de los nodos _worker_ iterando por cada grupo y actualizando los nodos uno a uno.
