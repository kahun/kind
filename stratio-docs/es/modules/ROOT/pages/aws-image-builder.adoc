...
. Añadir el binario generado en el punto `1` y el fichero de configuración `ecr-credential-provider-config.yaml`.
+
IMPORTANT: Requerido para versiones de kubernetes 1.27+
+
[source,console]
----
mkdir -p ansible/roles/providers/files
cp cloud-provider-aws/cmd/ecr-credential-provider/ecr-credential-provider ansible/roles/providers/files/
cat <<EOF >> ansible/roles/providers/files/ecr-credential-provider-config.yaml
...
[source,console]
----
cat <<EOF >> ansible/roles/providers/tasks/aws.yml
